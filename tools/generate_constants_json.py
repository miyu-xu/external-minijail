...
#   %struct.constant_entry { i8* getelementptr inbounds ([19 x i8], [19 x i8]* @.str.894, i32 0, i32 0), i32 ptrtoint ([9 x i8]* @.str.895 to i32) },
...
# Current output for that constant in bazel-out/.../minijail/tools/constants.json is:
    ...
    for line in ir:
        ...
        if '@syscall_table' in line or '@constant_table' in line:
            ...
            for entry in _TABLE_ENTRY_RE.findall(line):
                ...
                if not groups:
                    if not _IGNORED_ENTRY_CONTENTS.match(entry):
